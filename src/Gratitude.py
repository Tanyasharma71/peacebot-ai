...
from utils.config_loader import get, getint
...
# -------------------------------------------------
# Load path from config or fallback
DATA_FILE = get("gratitude", "data_file", fallback="gratitude_log.json")
MAX_GRATITUDE_ITEMS = getint("gratitude", "max_items", fallback=3)

# Ensure the data directory exists if using a relative path
DATA_DIR = os.path.dirname(DATA_FILE) or "."
os.makedirs(DATA_DIR, exist_ok=True)

# Ensure the gratitude file exists
if not os.path.exists(DATA_FILE):
    try:
        with open(DATA_FILE, "w", encoding="utf-8") as f:
            json.dump([], f)
        logger.info(f"Created new gratitude log file at {DATA_FILE}")
    except Exception as e:
        logger.error(f"Failed to initialize gratitude file: {e}")
